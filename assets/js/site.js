...
function initZhihuFollowersBadge() {
...
function initPublicationFilter() {
  ...
  function getFilteredPublications(filter) {
    ...
    return pubItems.filter(item => item.querySelector(`.tag-${filter}`));
  }
...
function initGallery() {
  ...
  function appendGalleryItems() {
    galleryPhotos.slice(renderedGalleryCount, visibleGalleryCount).forEach((photo, offset) => {
      ...
      item.innerHTML = `<img src="${thumbnailSrc(photo.src)}" alt="${photo.caption}" width="260" height="260" loading="${loading}" decoding="async" fetchpriority="${index === 0 ? "high" : "low"}"><span class="gallery-caption">${photo.caption}</span>`;
